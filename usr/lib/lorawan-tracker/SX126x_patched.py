...
### HARDWARE CONFIGURATION METHODS ###
    ...
    def setPins(self, reset: int, busy: int, irq: int = -1, txen: int = -1, rxen: int = -1, wake: int = -1) :
        ...
        self._irq_edge_ok = False   # True when add_event_detect succeeds; False = use polling in wait()
    ...
    def endPacket(self, timeout: int = TX_SINGLE) -> bool :
        ...
        if self._irq != -1 :
            ...
            try :
                ...
                self._irq_edge_ok = True
            except RuntimeError :
                self._irq_edge_ok = False
        return True
...
### WAIT, OPERATION STATUS, AND PACKET STATUS METHODS ###
    ...
    def wait(self, timeout: int = 0) -> bool :
        ...
        while irqStat == 0x0000 and self._statusIrq == 0x0000 :
            ...
            if self._irq == -1 or not getattr(self, '_irq_edge_ok', False) : irqStat = self.getIrqStatus()
